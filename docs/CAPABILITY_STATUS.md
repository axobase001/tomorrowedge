...
Authoritative status table for TomorrowEdge 1.1.10. Use this page when deciding
...
| TomorrowEdge GUI Client | experimental | Browser GUI v1.1 uses the shared cockpit view model, live-event API, React cockpit build serving, runtime screenshots, collapsed telemetry summary, task queue, workflow main area, command composer, session-source/connection badges, approval-history timeline, capability dashboard, dark-mode CSS variables, branded favicon/manifest mark, the `tedge client` / `npm run client` entrypoint, and `npm run e2e:cockpit` coverage. |
