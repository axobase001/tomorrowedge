...
Authoritative map for TomorrowEdge 1.2.10 README product promises. Use this
...
| orchestration-governance | README positions TomorrowEdge as a GUI orchestration layer for strong-agent governance, budget-constrained routing, and multi-agent coding workflows. | `src/core/goal/modelPlanner.ts`, `src/core/goal/goalParser.ts`, `src/core/routing/policies.ts`, `src/core/routing/router.ts`, `src/core/budget/budgetAllocator.ts`, `src/config/schema.ts` | `tests/unit/plannerPolicy.test.ts`, `tests/unit/routing.test.ts`, `tests/unit/strongAgentBudget.test.ts`, `tests/unit/agentGraph.test.ts` | #296, #297, #298, #299 |
